...
NMAP_PLATFORM=x86_64-apple-darwin19.6.0
...
# DEFS += -DMTRACE=1
CXXFLAGS = -no-cpp-precomp -g -O2 -Wall -fno-strict-aliasing $(DBGFLAGS) $(CCOPT)
CPPFLAGS = -I$(top_srcdir)/liblinear -I$(top_srcdir)/liblua -I$(top_srcdir)/libdnet-stripped/include  -I/usr/local/opt/openssl@1.1//include -I$(top_srcdir)/nbase -I$(top_srcdir)/nsock/include $(DEFS)
CFLAGS = -g -O2 -Wall $(DBGFLAGS) $(CCOPT)
STATIC =
LDFLAGS =  -L/usr/local/opt/openssl@1.1//lib -Lnbase -Lnsock/src/ $(DBGFLAGS) $(STATIC)
LIBS =  -lnsock -lnbase -lpcre -lpcap $(LIBSSH2_LIBS) $(OPENSSL_LIBS) $(ZLIB_LIBS) libnetutil/libnetutil.a $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a $(top_srcdir)/liblua/liblua.a $(top_srcdir)/liblinear/liblinear.a -lm 
...
$(TARGET): build-netutil build-liblinear     build-nsock build-nbase build-dnet build-lua \
	$(OBJS) main.o
	@echo Compiling nmap
	rm -f $@
...
build-lua: $(LIBLUADIR)/Makefile
	@echo Compiling liblua; cd $(LIBLUADIR) && $(MAKE) liblua.a CC="$(CC)" MYCFLAGS="$(CFLAGS) -DLUA_USE_MACOSX"
...
clean: clean-lua clean-liblinear   clean-dnet  \
...
distclean: distclean-lua distclean-liblinear  \
  distclean-dnet distclean-nping distclean-zenmap \
